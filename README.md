...
Registers a hybrid `collections` post type and `collection` taxonomy that allows editors to curate groups of PRC content sharing common themes, research initiatives, or special projects (e.g., the Religious Landscape Study). The post type and taxonomy are linked via the `prc/primitives` term data store (namespace `PRC\Primitives\TDS`), so each collection post has a corresponding taxonomy term used to tag associated content.
...
- Establishes a `prc/primitives` term data store relationship between the `collections` post type and the `collection` taxonomy — each collection post maps to a term, enabling content to be tagged and queried by collection
...
| `includes/class-content-type.php`                                       | Registers the `collections` post type, `collection` taxonomy, kicker meta, the `prc/primitives` term data store relationship, template part area, and query modification     |
...
| `init` (priority 20)                           | Action    | Registers the `collection` taxonomy and the `prc/primitives` term data store relationship after other PRC CPTs register on default `init`.                                                                                                                                                                     |
...
- `prc/block-utils` (via `prc-platform-core`) — `\PRC\Primitives\BlockUtils\load_blocks()` and `classNames()` used at runtime; blocks will not register if the function is unavailable
- `prc/primitives` term data store (`PRC\Primitives\TDS` namespace) — `\PRC\Primitives\TDS\add_relationship()` and `\PRC\Primitives\TDS\get_related_term()` must be available on `init`
